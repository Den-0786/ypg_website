...
const EventsManagement = ({ events = [], setEvents, theme }) => {
  ...
  const handleUpdateEvent = async () => {
    ...
    formData.append("participants", editingEvent.participants || 0);
              ...
              <div className="flex space-x-2">
                {event.status !== "deleted" ? (
                  <>
                    <button
                      onClick={() => {
                        // Parse the datetime fields for editing
        ...
        {editingEvent && (
          ...
          >
            <motion.div
              ...
              className={`${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6 border`}
            >
